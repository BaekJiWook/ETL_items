...
const loadData = async () => {
  loading.value = true;
  try {
    // const response = await axios.get("http://localhost:8082/api/itemsAll", {
    const response = await axios.get("http://52.79.139.134:8082/api/itemsAll", {
      params: {
...
<template>
  <div>
    ...
    <ul class="items-all-list" v-if="paginatedItems.length > 0">
      <li class="items" v-for="(item, index) in paginatedItems" :key="index">
        <div class="header">
          <h3>{{ item.prdctNm }}</h3>
